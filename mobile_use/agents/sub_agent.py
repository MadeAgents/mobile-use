...
"""
        ...
        prompt += """- If you want to type in some text, remember to click the input field first.
- If it shows an advertisement, you must use the action `wait`. Do not click!!!
- If it requests any permissions, you must use the action `wait`. Do not click!!!
- If the screenshot shows any terms and conditions, you must use the action `wait`. Do not click!!!
- If the screenshot shows any popups, you must use the action `wait`. Do not click!!!
- If the screenshot asks user preference or profile, you must use the action `wait`. Do not click!!!
- If the screenshot shows any other information that you think requires user attention, you must use the action `wait`. Do not click!!!
...
class Evaluator(SubAgent):
    def __init__(self, num_latest_screenshots: int = 0):
        super().__init__()
    ...
    def get_message(self, episodedata: EpisodeData) -> list:
        ...
        messages.append({
            ...
            "content": [
                {
                    "type": "text",
                    "text": "You are a helpful AI assistant for operating mobile phones. Your goal is to evaluate whether the user's instruction has been completed."
                }
            ]
        })
        ...
        prompt += "### Response requirements ###\n"
        prompt += "Carefully examine the information provided above to determine whether the user's instruction is completed or not. Provide your output in the following format:\n\n"

        prompt += "### Result ###\n"
        prompt += "Give your answer as \"Finished\" or \"Not Finished\".\n"
...
class AdvertisementDetector(SubAgent):
    def get_message(self, episodedata: EpisodeData) -> list:
        messages = []
        trajectory = episodedata.trajectory
        current_step = trajectory[-1]
        ...
        messages.append({
            ...
            "content": [
                {
                    "type": "text",
                    "text": "You are a helpful AI assistant for operating mobile phones. Your goal is to choose the correct actions to complete the user's instruction."
                }
            ]
        })
        ...
        prompt += "### Requirements ###\n"
        prompt += "You can choose from the following actions to complete the user's instruction:\n"
        prompt += "- `wait`: Wait for a while, used when there is an advertisement, countdown, or progress bar on the screen, or the screen is loading.\n"
        prompt += "- `abort`: Abort to require the user attention. It is used when the current screenshot applies for system permission, terms and conditions, asks user performance, or other information that requires user attention.\n"
        prompt += "- `other`: Other actions that are not need to wait or abort.\n"
        prompt += "If you think the current screenshot shows an advertisement, you should use the action `wait` to wait for the advertisement to disappear. You also need to provide the coordinate of the close button in the `wait` action if it can be closed.\n\n"
        ...
        prompt += "Carefully observe the current screenshot and think about the definition of each action. Put your thinking process here in one sentence.\n"
        prompt += "### Action ###\n"
        prompt += "You should output the action you choose to complete the user's instruction. The action should be one of the following: `wait`, `abort`, or `other`.\n"
        prompt += "### Bounding Box ###\n"
        prompt += "If you think the current screenshot shows an advertisement, you should use the action `wait`, and also output the bounding box of the close button here in the format of `[x1, y1, x2, y2]`. x1 and y1 are the coordinates of the top-left corner of the bounding box, and x2 and y2 are the coordinates of the bottom-right corner of the bounding box. If you think there is no close button, you should output `None`.\n"
    ...
    def parse_response(self, content: str, size: tuple[float, float], raw_size: tuple[float, float]):
        ...
        action = content.split("### Action ###")[-1].split("### Bounding Box ###")[0].strip()
        if "wait" in action:
            action_name = "wait"
        ...
        bounding_box = content.split("### Bounding Box ###")[-1].strip()
        if action_name == "wait" and bounding_box.lower() != "none":
            try:
                bounding_box = json.loads(bounding_box)
                assert isinstance(bounding_box, list) and len(bounding_box) == 4
                bounding_box = [round(point / size[i % 2] * raw_size[i % 2]) for i, point in enumerate(bounding_box)]
            except Exception as e:
                logger.warning("Failed to parse bounding box, error: %s, using default value (None).", e)
                bounding_box = None
        else:
            bounding_box = None
        
        return thought, action_name, bounding_box
